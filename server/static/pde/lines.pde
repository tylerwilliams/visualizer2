...
void handleSegment(Object segment) {
    ;
}
...
void draw() {
  if (javascript != null) {
  	...
  	if (javascript.getViewportWidth() != tamanhoWIDTH) {
  		tamanhoWIDTH = javascript.getViewportWidth();
  		shouldResize = true;
  	}
  	if (javascript.getViewportHeight() != tamanhoHEIGHT) {
  		tamanhoHEIGHT = javascript.getViewportHeight();
  		shouldResize = true;
  	}
  	if (shouldResize) {
  		size(tamanhoWIDTH, tamanhoHEIGHT);
  ...
  if ((javascript != null) && (didRegister == false)) {
